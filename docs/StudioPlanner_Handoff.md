...
31. **`studioLevel` value vocabulary = `LEVEL_ORDER` only (Victor, 2026-07-27)** — no studio-specific level names, no extension of `LEVEL_ORDER`, no parallel ordering; every value is orderable on assignment, so tier grouping and the cumulative filter needed no code change. NDCC fields stay verbatim-or-null; resolution unchanged at `studioLevel ?? syllabusLevel` (#28). CDTA divergences ride in `syllabusNotes`, never in `syllabusLevel` or `studioLevel`. Applied at `b864e9b` / `v0.6.0-data`. **Rider (Victor, same date): the unrecognised-vocabulary guard is RETAINED** — see the tracked-issues entry for the exact failure direction (it fails toward *shown to everyone*, not *hidden*) and its scope (drift defence, not #31-compliant data).
...
- ~~**`studioLevel` unrecognised-vocabulary handling**~~ — **resolved 2026-07-27** by decision #31 + its rider. The guard is **RETAINED**, not retired. #31 changes why it exists, not whether it is needed. **Exact mechanism (verified; Code's `v0.6.0-data` report and commit message stated this backwards):** `levelIndex` returns `-1`, which is **below** Beginners (0), so the value sorts **first** and `levelIndex(lvl) > levelIndex(maxLevel)` is never true — it is filtered out at **no** target level. **It fails toward "shown to everyone", not "hidden."** A misspelled `'Siver'` on a Gold figure would surface it in a Beginners session; the guard sends it to Needs Review instead. **Defends against future drift (hand-edits, Latin parse output), not against #31-compliant data — do not delete as dead defensive code.** Note also the Needs Review predicate is `effectiveLevel == null`, **not** `syllabusLevel == null`; those diverged for the 10 placed figures.
